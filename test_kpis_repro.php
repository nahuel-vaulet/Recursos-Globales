...
try {
    echo "Testing Updated Dashboard SQL...\n";

    // Replicating the logic from get_dashboard_kpis.php (NEW VERSION)
    ...
    $sql_tasks = "SELECT 
              ...
              WHERE (estado = 'En Curso')
                 OR (importancia = 'Alta' AND estado = 'Pendiente' AND fecha_vencimiento BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 1 DAY))";
    ...
    // Also check raw tasks table to see what we MIGHT be missing
    echo "\n--- Raw Tasks Scan (Due Today/Tomorrow) ---\n";
    $sql_raw = "SELECT id_tarea, titulo, estado, importancia, fecha_vencimiento FROM tareas_instancia WHERE fecha_vencimiento BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 2 DAY)";
    $raw = $pdo->query($sql_raw)->fetchAll(PDO::FETCH_ASSOC);
    print_r($raw);
